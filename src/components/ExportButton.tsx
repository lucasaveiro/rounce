  ...
  const [loading, setLoading] = useState(false);
    ...
    const exportAndUpload = async () => {
      ...
      const blob: Blob = await new Promise((resolve) => {
        const opt: { margin: number; filename: string } = { margin: 10, filename: `rounce-plan-${planId}.pdf` };
    ...
    const path = `${userRes.user.id}/${planId}.pdf`;
      const { error } = await supabase.storage.from('plans').upload(path, blob, { upsert: true, contentType: 'application/pdf' });
